 ...
 */
public class AcSnakeBar extends Andmt {
    private float _w=0;
    private float _h=0;
    private float _sw=0;
    private float _xw=0;
    ...
    public AcSnakeBar(Context context, float shangWanLen,float xiaWanLen,float width,float height,boolean isAnnote) {
        ...
        _l= Utils.MMP(get_d(),_h / _w);
        _C = Utils.T_C(_h, width);
    ...
    public void annotate() {
        ...
        if (isAnnote) {
            ...
            an.addtext("h1",String.valueOf(_h));
            an.addtext("total", String.valueOf(_sw+_C+_xw));
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isAnnote) {
            an.autoDrawAnnotation(canvas, 5, 20, 30);
        }
